...
export default function EventPage({ initialEvent, initialListings }: EventPageProps) {
  ...
  useEffect(() => {
    if (!initialEvent) fetchEvent()
  }, [params.id])

  useEffect(() => {
    if (event) {
      // Skip initial fetch if we have SSR data and no user changes yet
      if (initialListings && sort === 'price_asc' && !filters.minPrice && !filters.maxPrice && !filters.section && !filters.deliveryMethod) return
      fetchListings()
    }
